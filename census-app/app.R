...
library(maps)
library(mapproj)
setwd("C:\\Users\\Administrator\\OneDrive\\Pulpit\\census-app")
source("helpers.R")
...
server <- function(input, output) {
  output$mapa <- renderPlot({
    ...
    percent_map(var = data, color =color, legend.title = legend.title, max = input$zakres[2], min = input$zakres[1])
  })
